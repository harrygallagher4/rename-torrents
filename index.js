...
function processMovie(file) {
  ...
  if (IGNORE_FILES.includes(torrent.title.toLowerCase())) {
    console.log(`\tIgnoring file: ${file}`);
    console.log(`\tIf this is an error, remove '${torrent.title.toLowerCase()}' from IGNORE_FILES`);
    return;
  }
...
function processTV(file) {
  ...
  try {
    ...
    console.log(`MOVING:\t${file} => ${episodePath}`);
      ...
      tl.allFilesWithCategory.forEach((type, file) => {
        if (type == 'MOVIES') {
          ...
          if (file.toLowerCase().includes('sample')) {
            console.log(`[MOVIES] Detected sample file, skipping: ${file}`);
            return;
          }
